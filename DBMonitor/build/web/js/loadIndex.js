...
function consult() {
    Proxy.getTablespaces(function (res) {
        ...
        Controller.createPieChart(res, index);

        Storage.store("binnacle", storage);
        Storage.store("last",{});
        Storage.store("last",storage);
...
function recuperarTabla(index1){
    Proxy.getTablespaces(function (res) {
        //*** local - sotrage ***
        var storage = {} // {a1:{size:0,free:[],used:[]}}
        storage = Storage.retrieve("last");
        if (storage == null) {
            storage = {}
        }
        console.log(storage)
        ...
        index=index1;
        Controller.createPieChart(res, index1)
